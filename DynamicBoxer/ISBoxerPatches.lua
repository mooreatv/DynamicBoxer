...
function isboxer.ClearMembers()
  ...
  for i = 1, max, 1 do
    table.remove(isboxer.CharacterSet.Members)
  end
...
function isboxer.CheckBoundCombo(key, combo)
  ...
  if (action and action ~= "") then
    isboxer.Warning("Modifier conditions used for targeting on '" .. key .. "' may not work because " .. combo .. " is bound to " ..
                      action)
  end
end
...
function isboxer.LoadBinds()
  if (isboxer.CharacterSet.LoadBinds or isboxer.Character.LoadBinds) then
    ...
    if (isboxer.Character.LoadBinds) then
      ...
      isboxer.Output("WoW Macros for Character '" .. isboxer.Character.Name .. "' in Set '" .. isboxer.CharacterSet.Name ..
                       "' Loaded.")
      if (isboxer.Character.ActualName ~= "*" and isboxer.Character.ActualName:upper() ~= GetUnitName("player"):upper()) then
        ...
        StaticPopupDialogs["ISBOXER_WRONGCHARACTER"] = {
          text = "Character in wrong window? ISBoxer expected " .. isboxer.Character.ActualName .. " but got " ..
                                                           GetUnitName("player") .. ". Some functionality may not work correctly!",
          button1 = OKAY,
          timeout = 0,
          whileDead = true,
          hideOnEscape = true
